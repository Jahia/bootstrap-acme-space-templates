...
<c:set var="cssClassName" value="${currentNode.properties['cssClassName'].string}"/>
<c:set var="sectionId" value="${currentNode.properties['sectionId'].string}"/>

<c:set var="classAttr"/>
<${sectionType} <c:if test="${! empty cssClassName}"> class="${cssClassName}"</c:if>  <c:if test="${! empty sectionId}"> id="${sectionId}"</c:if>>
